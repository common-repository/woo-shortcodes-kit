...
<button class="wshkshtboxesbtn" style="width:150px;" type="button" onclick="myFunctionglobalsal()" onmouseout="outFuncglobalsal()">
  <span class="tooltiptext" id="myTooltipglobalsal"><?php esc_html_e( 'Copy to Clipboard', 'wshk' ); ?></span>
...
function myFunctionglobalsal() {
  ...
  tooltipglobalsal.innerHTML = "<?php esc_html_e( 'Copied:', 'wshk' ); ?> " + copyText.value;
}

function outFuncglobalsal() {
  var tooltipglobalsal = document.getElementById("myTooltipglobalsal");
  tooltipglobalsal.innerHTML = "<?php esc_html_e( 'Copy to Clipboard', 'wshk' ); ?>";
}
...
<br><br>
<p style="padding-left:30px;"><?php esc_html_e( 'This shortcode shows total store sales, adding orders with status processing, completed and on hold', 'wshk' ); ?></p>
<br><br><br>
        </div>